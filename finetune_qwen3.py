...
Features:
- Load dataset from local JSON/JSONL files containing Text-to-SQL examples
- Generate training prompts from rules files and database schemas
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune Qwen3 models with LoRA/QLoRA for Text-to-SQL tasks"
    )
    ...
    parser.add_argument("--rules_file", type=str, required=True, 
                       help="Path to rules JSON file containing Text-to-SQL examples")
    parser.add_argument("--instruction", type=str, 
                       default="Read the question and output only the definitional rules that apply.",
    ...
    parser.add_argument("--skip_no_def", action="store_true", 
                       help="Skip training samples that don't have definitional rules")
...
def is_definitional_rule(rule: Dict) -> bool:
    """
    Check if a rule is a definitional rule (for Text-to-SQL tasks).
    
    Args:
        rule (Dict): Rule dictionary containing type and operation information
        
    Returns:
        bool: True if the rule is definitional, False otherwise
    """
    # Note: The data uses "definational" (typo) instead of "definitional"
    return str(rule.get("type", "")).lower() in ["definitional", "definational"]


def build_io_pair(instruction: str, schema: str, question: str, definitional_rules: List[str]) -> str:
    """
    ...
    Args:
        ...
        definitional_rules (List[str]): List of definitional rules with condition and operation
        
    Returns:
        str: Formatted training prompt with input and expected output
    """
    # Format the target rules with proper structure
    if definitional_rules:
        target = "\n\n".join([f"Rule {i+1}:\n{rule}" for i, rule in enumerate(definitional_rules)])
    else:
...
def get_prompts_from_rules(rules_json_path: str, instruction: str, skip_no_def: bool, db_root_path: str, schema_rows: int) -> List[Dict[str, str]]:
    """
    Generate training prompts from rules file and database schemas.
    
    Args:
        rules_json_path (str): Path to rules JSON file
        instruction (str): Instruction prompt for the model
        skip_no_def (bool): Whether to skip samples without definitional rules
    ...
    for item in iter_rules_items(rules_json_path):
        ...
        # Extract definitional rules with full condition and operation
        definitional_rules = [
            f"Condition: {r.get('condition', '').strip()}\nOperation: {r.get('operation', '').strip()}"
            for r in rules 
            if is_definitional_rule(r) and r.get("condition") and r.get("operation")
        ]
        
        # Skip samples without definitional rules if requested
        if skip_no_def and not definitional_rules:
            continue
        ...
        text = build_io_pair(instruction, schema, question, definitional_rules)
