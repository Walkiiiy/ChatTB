...
class RuleProcesser:
    ...
    def generate_comment_prompt_testRules(self, question,rules):
        base = "the database's schema is shown above, now you are required to "
        base += "solve the following question related to the database by generating SQLite query."
    ...
    def getPrompt_testRules(self,evalObj):
        systemPrompt='''
            You are a helpful assistant that writes valid SQLite queries.
            '''
        schema_prompt=self.generate_schema_prompt(os.path.join(self.db_root_path, evalObj['db_id'], evalObj['db_id'] + '.sqlite'))
        comment_prompt=self.generate_comment_prompt_testRules(evalObj['question'], {'\n'.join(evalObj['rules']) if len(evalObj['rules'])>0 else ''})
        Prompt=f'''
            you will be given database schema, a question related to the database and some amends.
            you should generate a SQLite query that solve the question with the help of amends.
    ...
    def testRules(self,evalObj):
        print("testing rules...")
        systemPrompt,prompt=self.getPrompt_testRules(evalObj)
...
load_path='/home/walkiiiy/ChatTB/Process_document/V14/part_5.json'
dump_path='/home/walkiiiy/ChatTB/Process_document/V14/part_5.json'
...
getRulesMCPserver_path='/home/walkiiiy/ChatTB/Process_document/V14/src/MCPserver_getRules.py'
testMCPserver_path='/home/walkiiiy/ChatTB/Process_document/V14/src/MCPserver_testSql.py'
tableSchema_path='/home/walkiiiy/ChatTB/Bird_dev/dev_tableAnalyze.json'
Processer=RuleProcesser(load_path=load_path,
                            ...
                            ruleMCPserver_path=getRulesMCPserver_path
                            )

# Processer.amend_loop(maxRefineloop=20)
Processer.rule_loop()
# linked_schema=Processer.fetch_linked_schema(Processer.evalRes['1']['db_id'],Processer.evalRes['1']['amends'][-1])
# print(linked_schema)
