"""
Text2SQL DeepSeek Experiment Script

This script tests the text2sql capability using DeepSeek API with the BIRD dataset.
It loads questions from train.json, generates SQL using the DeepSeekLLMClient, and verifies
results using SQLTestComparator.

Usage:
    python text2sql_deepseek_experiment.py
"""
...
load_dotenv()
class Text2SQLDeepSeekExperiment:
    """
    Text2SQL experiment class for testing DeepSeek model capability.
    """
    
    def __init__(self, 
                 train_json_path: str = base_path + "/Bird_dev/res.json",
                 db_root_path: str = base_path + "/Bird_dev/dev_databases",
                 output_json_path: str = base_path + "/Bird_dev/EXP_deepseek_rules.json",
                 api_key: str = None,
                 model: str = "deepseek-coder",
                 # Alternative paths for different datasets:
                 # train_json_path: str = base_path + "/Bird_train/train.json",
                 # db_root_path: str = base_path + "/Bird_train/train_databases",
                 # output_json_path: str = base_path + "/Bird_train/EXP_deepseek.json",
                 #
                 # train_json_path: str = base_path + "/Spider_train/train.json",
                 # db_root_path: str = base_path + "/Spider_train/database",
                 # output_json_path: str = base_path + "/Spider_train/EXP_deepseek.json",
                 #
                 # train_json_path: str = base_path + "/Spider_dev/res.json",
                 # db_root_path: str = base_path + "/Spider_dev/database",
                 # output_json_path: str = base_path + "/Spider_dev/EXP_deepseek.json",
                 ):
        """
        Initialize the Text2SQL DeepSeek experiment.
        ...
        self.llm_client = DeepSeekLLMClient(api_key=api_key, model=model)
        ...
        self.logger.info("Text2SQL DeepSeek experiment initialized successfully")
    ...
    def _process_single_question(self, question_id: str, current_idx: int, total: int):
        ...
        rules = question_data.get('rules', '')  # target rules
        ...
        # Generate SQL using DeepSeek LLM
        system_prompt, user_prompt = self._generate_sql_prompt(question, schema, rules)
...
def main():
    ...
    try:
        ...
        experiment = Text2SQLDeepSeekExperiment()
        ...
        if not experiment.llm_client.test_connection():
