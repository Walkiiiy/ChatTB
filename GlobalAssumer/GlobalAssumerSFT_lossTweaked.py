...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tokenized_db_path", type=str, 
                       default="/home/ubuntu/walkiiiy/ChatTB/Bird_dev/tokenizedDB.json",
                       help="Path to tokenizedDB.json file containing schema tokens")
    parser.add_argument("--schema_loss_weight", type=float, default=3.0,
                       help="Loss weight multiplier for schema tokens")
...
def load_schema_tokens(tokenized_db_path: str, db_ids: Optional[List[str]] = None) -> set:
    """
    Load schema tokens from tokenizedDB.json file.
    
    Args:
        tokenized_db_path (str): Path to the tokenizedDB.json file
        db_ids (Optional[List[str]]): List of database IDs to load tokens for.
                                      If None, loads tokens from all databases.
    
    Returns:
        set: Set of unique token IDs from the specified databases
    """
    if not os.path.exists(tokenized_db_path):
        print(f"⚠️  Warning: Tokenized DB file not found at {tokenized_db_path}")
        print("   Schema-aware loss weighting will be disabled.")
        return set()
    
    try:
        with open(tokenized_db_path, "r", encoding="utf-8") as f:
            tokenized_db = json.load(f)
        
        schema_tokens = set()
        
        # If specific db_ids provided, only load those
        if db_ids is not None:
            for db_id in db_ids:
                if db_id in tokenized_db and "tokens" in tokenized_db[db_id]:
                    schema_tokens.update(tokenized_db[db_id]["tokens"])
                else:
                    print(f"⚠️  Warning: Database '{db_id}' not found in tokenizedDB.json")
        else:
            # Load tokens from all databases
            for db_id, db_data in tokenized_db.items():
                if "tokens" in db_data:
                    schema_tokens.update(db_data["tokens"])
        
        print(f"✅ Loaded {len(schema_tokens)} unique schema tokens from {len(db_ids) if db_ids else len(tokenized_db)} database(s)")
        return schema_tokens
    
    except Exception as e:
        print(f"⚠️  Warning: Failed to load schema tokens from {tokenized_db_path}: {e}")
        print("   Schema-aware loss weighting will be disabled.")
        return set()
...
class CustomSFTTrainer(SFTTrainer):
    """
    Custom SFTTrainer with enhanced logging and schema-aware loss reweighting.
    """
    def __init__(self, schema_tokens=None, schema_loss_weight: float = 3.0, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.schema_tokens = schema_tokens or set()
        self.schema_loss_weight = schema_loss_weight
        self.loss_fct = nn.CrossEntropyLoss(reduction="none")  # 逐 token loss
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 构造权重
        weights = torch.ones_like(shift_labels.view(-1), dtype=torch.float, device=shift_labels.device)
        schema_token_count = 0
        if self.schema_tokens:
            shift_labels_flat = shift_labels.view(-1)
            for tok in self.schema_tokens:
                mask = shift_labels_flat == tok
                count = mask.sum().item()
                if count > 0:
                    schema_token_count += count
                    weights[mask] = self.schema_loss_weight
            
            # 如果本条数据中存在schema tokens，输出提示
            step = self.state.global_step
            if schema_token_count > 0 and hasattr(self, "state"):
                print(f"🔍 [Step {step}] Schema tokens detected: {schema_token_count} tokens with {self.schema_loss_weight}x loss weight")
            else:
                print(f"🔍 [Step {step}] 0 schema tokens detected")
...
class CollatorMaskAfterDelimiter:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        # Determine max_length to use
        max_len = self.max_length if self.max_length is not None else self.tokenizer.model_max_length
        ...
        if "input_ids" in features[0]:
            ...
            if "attention_mask" in features[0]:
                ...
                attn_list = [attn[:max_len] for attn in attn_list]
...
def main() -> None:
    ...
    print(f"📏 Set tokenizer max length to: {args.max_seq_length}")
    ...
    # Extract unique database IDs from rules file
    print("\n🔍 Extracting database IDs from training data...")
    db_ids_in_training = set()
    try:
        with open(args.rules_file, "r", encoding="utf-8") as f:
            ...
            if isinstance(rules_data, list):
                for item in rules_data:
                    if "db_id" in item:
                        db_ids_in_training.add(item["db_id"])
            elif isinstance(rules_data, dict):
                for item in rules_data.values():
                    if "db_id" in item:
                        db_ids_in_training.add(item["db_id"])
        print(f"✅ Found {len(db_ids_in_training)} unique database(s) in training data: {sorted(db_ids_in_training)}")
    except Exception as e:
        print(f"⚠️  Warning: Could not extract database IDs: {e}")
        print("   Will load tokens from all databases in tokenizedDB.json")
        db_ids_in_training = None
    
    # Load schema tokens for loss reweighting
    print("\n📊 Loading schema tokens for loss reweighting...")
    schema_tokens = load_schema_tokens(args.tokenized_db_path, list(db_ids_in_training) if db_ids_in_training else None)
    if schema_tokens:
        print(f"   Schema loss weight: {args.schema_loss_weight}x")
    else:
        print("   Schema-aware loss reweighting disabled")
    ...
    data_collator = CollatorMaskAfterDelimiter(
        tokenizer=tokenizer, 
        delimiter=response_template,
        max_length=args.max_seq_length
    )
    trainer = CustomSFTTrainer(
        schema_tokens=schema_tokens,
        schema_loss_weight=args.schema_loss_weight,
        model=model,
        train_dataset=ds,
    ...
    if schema_tokens:
        print(f"   Using schema-aware loss with {len(schema_tokens)} tokens (weight: {args.schema_loss_weight}x)")
