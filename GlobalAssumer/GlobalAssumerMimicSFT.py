...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--refiner_model", type=str, default="Process_model/models--Qwen3-8B")
...
class CustomSFTTrainer(SFTTrainer):
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        # first, do inference with inputs
        response = model.generate(inputs["input_ids"], max_new_tokens=65535, temperature=0.0, do_sample=False)
        refined_inputs = self.refine_inputs(inputs, response)

        # then, do training with refined inputs
        loss = super().training_step(model, refined_inputs, num_items_in_batch)
        ...
        if hasattr(self, "state") and self.state.global_step % 20 == 0:
            ...
            print('='*60,'\n','refined_inputs','\n',self.tokenizer.decode(refined_inputs["input_ids"][0], skip_special_tokens=True))
            
        return loss
    
    def refine_inputs(self, inputs, response):
        """
        Refine the inputs to the model.
        """
        ground_truth=self.tokenizer.decode(inputs["input_ids"][0], skip_special_tokens=True)
        ground_truth_rules=ground_truth.split("generated rules:\n")[-1].strip()
        generated=self.tokenizer.decode(response[0], skip_special_tokens=True)
        generated_rules=generated.split("generated rules:\n")[-1].strip()
        if ground_truth_rules==ground_truth.strip():
            print("TRUNKED!")
            raise ValueError("TRUNKED!")
        if generated_rules==generated.strip():
            print("TRUNKED!")
            raise ValueError("TRUNKED!")
        refine_prompt=f'''
        You are a helpful assistant that refines the generated rules to align with the ground truth rules.
        Please refine the generated rules to align with the ground truth rules.
        If the generated rules contain all the information ground_truth_rules contains, please return the generated rules directly.
        If the ground_truth_rules contains more information than the generated rules, add those information to the generated rules to align with the ground truth rules.
        If the ground_truth_rules and the generated rules are similar but in different meanings, align the meanings to the ground truth rules using minimal changes.
        The refined rules should be most similar to generated_rules but contains all the information in ground_truth_rules.
        The table and column names are most important, make sure they are aligned with the ground truth rules.
        Only return the refined rules, no other text.
        The generated rules are:
        {generated_rules}
        The ground truth rules are:
        {ground_truth_rules}
        the refined rules are:
        '''
        refined_rules=self.refiner.chat(refine_prompt)
        
        simulated_batch = [{"text": refined_rules}]
    
        # 使用你的 data collator 来处理，这样会应用相同的 masking 逻辑
        refined_inputs = self.data_collator(simulated_batch)
...
def main() -> None:
    ...
    refiner=LLMClient(model_path=args.refiner_model)
