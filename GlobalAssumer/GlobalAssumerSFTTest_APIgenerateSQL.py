...
1. Loading the fine-tuned model with LoRA adapter
2. Randomly selecting samples from a dataset (supports condensed_rules.json format)
...
4. Testing THREE approaches:
   ...
   B. Generate SQL using DeepSeek WITH dataset rules (ground truth)
...
Usage:
    python AssumerSFTTest_APIgenerateSQL.py --base_model /path/to/base/model --adapter_path /path/to/adapter --dataset /path/to/condensed_rules.json --num_samples 10
"""
...
class SFTResultsAnalyzer:
    ...
    def generate_visualizations(self, results: List[Dict], statistics: Dict) -> None:
        ...
        fig.suptitle('SFT API SQL Generation Test Results', fontsize=16, fontweight='bold')
    ...
    def generate_enhanced_json_report(self, results: List[Dict], statistics: Dict, test_config: Dict) -> str:
        ...
        enhanced_data = {
            "metadata": {
                ...
                "test_type": "SFT API SQL Generation Test",
                "version": "2.0"
            },
    ...
    def generate_summary_report(self, statistics: Dict, analysis: Dict) -> str:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("SFT API SQL Generation Test - Summary Report\n")
            f.write("=" * 50 + "\n\n")
...
class SFTResultsTester:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        log_file = self.output_dir / "sft_api_test.log"
        ...
        # Also set up root logger to capture all logs
...
### Now process the following:
            ...
            system_prompt, user_prompt = build_sql_generation_prompt_with_dataset_rules(question, schema, dataset_rules_text)
    ...
    def save_results(self):
        ...
        try:
            ...
            self.logger.info(f"   - {self.output_dir}/sft_api_test.log (test log)")
